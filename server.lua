...
RegisterCommand('ac-unban', function(source, args, rawCommand)
    ...
    if (src <= 0) then
        ...
        if #args == 0 then 
            ...
            print('^3[^6Dead-Anticheat^3] ^1Not enough arguments...');
            return; 
        end
        local banID = args[1];
        if tonumber(banID) ~= nil then
            ...
            if playerName then
                print('^3[^6Dead-Anticheat^3] ^0Player ^1' .. playerName 
                .. ' ^0has been unbanned from the server by ^2CONSOLE');
                TriggerClientEvent('chatMessage', -1, '^3[^6Dead-Anticheat^3] ^0Player ^1' .. playerName 
                .. ' ^0has been unbanned from the server by ^2CONSOLE'); 
            else 
                -- Not a valid ban ID
                print('^3[^6Dead-Anticheat^3] ^1That is not a valid ban ID. No one has been unbanned!'); 
            end
        end
        return;
    end 
    if IsPlayerAceAllowed(src, "Dead-Anticheat.ACban") then 
        if #args == 0 then 
            -- Not enough arguments
            TriggerClientEvent('chatMessage', src, '^3[^6Dead-Anticheat^3] ^1Not enough arguments...');
            return; 
        end
        local banID = args[1];
        if tonumber(banID) ~= nil then 
            ...
            if playerName then
                ...
                -- Not a valid ban ID
                TriggerClientEvent('chatMessage', src, '^3[^6Dead-Anticheat^3] ^1That is not a valid ban ID. No one has been unbanned!'); 
            end
        else 
            -- Not a valid number
            TriggerClientEvent('chatMessage', src, '^3[^6Dead-Anticheat^3] ^1That is not a valid number...'); 
        end
    end
end)
...
RegisterCommand("latest", function(source, args, rawCommand) 
    ...
    for i = 1, #latestUsers do 
        ...
        TriggerClientEvent('chatMessage', source, "^5[^1Dead-Anticheat^5] ^3Player ^3[^4".. tostring(user) .. "^3] ^4" .. 
...
Citizen.CreateThread(function()
    while true do 
        ...
        for _, id in pairs(GetPlayers()) do 
            if isBanned(id) then 
                -- Banned, kick em 
                DropPlayer(id, "[Dead-Anticheat] " .. bans[tostring(playerIP)]);
            end
        end
    end
end)
...
function OnPlayerConnecting(name, setKickReason, deferrals)
    deferrals.defer();
    local src = source;
    print("[Dead-Anticheat] Checking ban data for "..GetPlayerName(src));
    local banned = false;
    ...
    if ban then 
        ...
        if string.find(reason, "[Dead-Anticheat]") then 
            printMessage = "" 
        else 
            printMessage = "[Dead-Anticheat] " 
        end 
...
RegisterCommand("acban", function(source, args, raw)
    ...
    if IsPlayerAceAllowed(src, "Dead-Anticheat.ACban") then 
        ...
        if #args < 2 then 
            ...
            TriggerClientEvent('chatMessage', source, "^5[^1Dead-Anticheat^5] ^1ERROR: You have supplied invalid amount of arguments... " ..
        ...
        if ExtractIdentifiers(args[1]) ~= nil then 
            ...
            DropPlayer(id, "[Dead-Anticheat]: Banned by player " .. GetPlayerName(src) .. " for reason: " .. reason);
        else 
            -- Not a valid player supplied 
            TriggerClientEvent('chatMessage', source, "^5[^1Dead-Anticheat^5] ^1ERROR: There is no valid player with that ID online... " ..
                "^2Proper Usage: /acban <id> <reason>");
        end
    end
end)
...
AddEventHandler("Anticheat:NoClip", function(distance)
    if Config.Components.AntiNoclip and not IsPlayerAceAllowed(source, "Anticheat.Bypass") then
        ...
        if counter[ids.steam] ~= nil and counter[ids.steam] >= Config.NoClipTriggerCount then 
            ...
            DropPlayer(id, "[Dead-Anticheat]: " .. Config.Messages.NoClipTriggered)
        end 
        Wait(6000);
        counter[ids.steam] = counter[ids.steam] - 1;
    end
...
AddEventHandler('chatMessage', function(source, name, msg)
    ...
    if (name ~= realName) then 
        if Config.BanComponents.AntiFakeMessage then 
            BanPlayer(id, "[Dead-Anticheat]: " .. Config.Messages.ChatMessageTriggered)
        ...
        DropPlayer(id, "[Dead-Anticheat]: " .. Config.Messages.ChatMessageTriggered)
    end
end)
  ...
  RegisterServerEvent(BlacklistedEvents[i])
    AddEventHandler(BlacklistedEvents[i], function()
        ...
        if Config.BanComponents.AntiBlacklistedEvent then 
            BanPlayer(id, "[Dead-Anticheat]: " .. reason);
      ...
      DropPlayer(id, "[Dead-Anticheat]: " .. reason)
    end)
end
...
AddEventHandler("entityCreating",  function(entity)
    ...
    if (model) then 
        if (owner ~= nil and owner > 0) then
            ...
            DropPlayer(owner, "[Dead-Anticheat]: " .. reason);
        end
        CancelEvent()
        cancelled = true
    end
...
CreateThread(function()
    if Config.Components.ModMenuChecks then
        ...
        for i = 1, GetNumResources() do
            ...
            if resource_name ~= GetCurrentResourceName() then
                for k, v in pairs({'fxmanifest.lua', '__resource.lua'}) do
                    ...
                    if data and type(data) == 'string' and string.find(data, 'acloader.lua') ~= nil then
                       -- data = data:lower()
                        local removed = string.gsub(data, 'client_script "%@Dead%-anticheat%-master%/acloader.lua"', "")
        ...
        if added then
            print('[Dead-AC] Uninstall Mod-Menu-Checks | Modified 1 or more resources. It is required to restart your server so these changes can now take place.')
        end
    end
end)
    ...
    AddEventHandler("ANTICHEAT:CHECKRESOURCES", function(givenList)
        ...
        for _, resource in ipairs(givenList) do
            if not validResourceList[resource] then
                BanPlayer(source, "[Dead-Anticheat]: " .. Config.Messages.UnauthorizedResources:gsub("{RESOURCE}", resource));
            end
        end
    end)
end
